...
const ResumeSection = () => {
  ...
  return (
    <div className="relative w-full bg-transparent mt-16 md:mt-10">
      ...
      {/* Centered content */}
      <div className="relative max-w-6xl mx-auto text-center pt-6 flex flex-col items-center px-4 sm:px-6 md:px-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 my-12 items-center">
          {cv.map((c, index) => (
            <div
              key={index}
              className={`md:max-w-72 mx-8 md:mx-0 ${index > 0 ? "hidden md:block" : ""}`}
            >
              <Link to={`/templates`}>
                <img
                  src={c.img}
